...
void IMU_RENDER::renderStartScreen(int index, String name, bool sdCardExists) {
...
void IMU_RENDER::renderStatistics(MeasurementMetadata& metadata, MeasurementConfig& config, MeasurementData& data) {
    ...
    // M5.Lcd.printf("%dhz (current: %dhz)", config.setHz, metadata.hz);
    M5.Lcd.printf("%dhz", metadata.hz);
    ...
    // M5.Lcd.printf("%d saved (%d batches)", metadata.measurementsIndex, metadata.measurementsIndex /
    // config.saveAtNDataPoints); M5.Lcd.setCursor(0, 140); M5.Lcd.printf("%d total", data.iterator);
